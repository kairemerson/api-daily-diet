import { PrismaClient, Role, Goal, PatientStatus } from "@prisma/client"
...
async function main() {
  ...
  for (let i = 0; i < 10; i++) {
    const nutritionist =
      nutritionists[i % nutritionists.length].nutritionistProfile!
  ...
  for (let i = 0; i < patients.length; i++) {
    ...
    if (i === patients.length - 1) continue
    ...
    const items = await Promise.all(
      ["Café da manhã", "Almoço", "Jantar"].map((name, index) =>
